...
-- =========================================================================
-- =========================================================================
--   APPROACH 2 — STAR SCHEMA AS VIEWS (NO PHYSICAL TABLES)
--   Everything below computes live from silver.RawData.
--   Named with a "vw_" prefix (not "DIM.../FactSales") so there is
--   no naming collision with the table-based objects above.
-- =========================================================================
-- =========================================================================
...
SELECT
    ROW_NUMBER() OVER (ORDER BY s.CustomerID) AS CustomerKey,
    s.CustomerID,
    s.CustomerName,
    s.Segment
FROM (SELECT DISTINCT CustomerID, CustomerName, Segment FROM silver.RawData) s;
GO
...
SELECT
    ROW_NUMBER() OVER (ORDER BY s.ProductID) AS ProductKey,
    s.ProductID,
    s.ProductName,
    s.Category,
    s.SubCategory
FROM (SELECT DISTINCT ProductID, ProductName, Category, SubCategory FROM silver.RawData) s;
GO

-- =========================================================
-- DIM GEOGRAPHY (view)
-- =========================================================
CREATE OR ALTER VIEW gold.vw_DIMGeography AS
SELECT
    ROW_NUMBER() OVER (ORDER BY s.Country, s.City, s.State, s.PostalCode, s.Region) AS GeographyKey,
    s.Country,
    s.City,
    s.State,
    s.PostalCode,
    s.Region
FROM (SELECT DISTINCT Country, City, State, PostalCode, Region FROM silver.RawData) s;
GO

-- =========================================================
-- DIM SHIPPING (view)
-- =========================================================
CREATE OR ALTER VIEW gold.vw_DIMShipping AS
SELECT
    ROW_NUMBER() OVER (ORDER BY s.ShipMode) AS ShippingKey,
    s.ShipMode
FROM (SELECT DISTINCT ShipMode FROM silver.RawData) s;
GO
...
-- =========================================================
-- FACT SALES (view) — joins the dimension views above
-- =========================================================
...
SELECT
    ...
    s.Profit
FROM silver.RawData s
LEFT JOIN gold.vw_DIMCustomer c
    ON s.CustomerID = c.CustomerID
LEFT JOIN gold.vw_DIMProduct p
    ON s.ProductID = p.ProductID
LEFT JOIN gold.vw_DIMGeography g
    ON  s.Country    = g.Country
    AND s.City       = g.City
    AND s.State      = g.State
    AND s.PostalCode = g.PostalCode
    AND s.Region     = g.Region
LEFT JOIN gold.vw_DIMShipping sh
    ON s.ShipMode = sh.ShipMode
WHERE s.HasMissingValue = 0
  AND s.HasInvalidValue = 0;
GO